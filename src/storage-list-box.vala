    ...
    public class StorageListBox : Gtk.ListBox
    {
        ...
        public void set_select_mode(bool select_mode)
        {
            if(select_mode)
            {
                ...
                if(root)
                    ((StorageView) (GLib.Application.get_default() as Application).get_window().get_views()[Views.STORAGE]).get_action_bar().show_root();
                else if(actual_parent_type == StorageItemType.TRASH)
                    ((StorageView) (GLib.Application.get_default() as Application).get_window().get_views()[Views.STORAGE]).get_action_bar().show_trash();
                else if(actual_parent_type != StorageItemType.TRASHFILE && actual_parent_type != StorageItemType.TRASHSUBFILE)
                    ((StorageView) (GLib.Application.get_default() as Application).get_window().get_views()[Views.STORAGE]).get_action_bar().show_common();
        ...
        private void on_selected_rows_changed()
        {
            ...
            if(get_selected_rows().length() > 0)
                ((StorageView) (GLib.Application.get_default() as Application).get_window().get_views()[Views.STORAGE]).get_action_bar().set_sensitive_all(true);
            else
                ((StorageView) (GLib.Application.get_default() as Application).get_window().get_views()[Views.STORAGE]).get_action_bar().set_sensitive_all(false);
        }
