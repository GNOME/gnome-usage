...
namespace Usage {
    ...
    public class DiskGraphTable : Rg.Table
    {
        ...
        public DiskGraphTable ()
        {
            var settings = Settings.get_default();
            set_timespan (settings.graph_timespan * 1000);
            set_max_samples (settings.graph_max_samples);

            var column_download = new Rg.Column("READ", Type.from_name("gdouble"));
            add_column(column_download);
            var column_upload = new Rg.Column("WRITE", Type.from_name("gdouble"));
        ...
        bool update_data()
        {
            ...
            iter.set (COLUMN_READ_ID, disk_read, -1);
            iter.set (COLUMN_WRITE_ID, disk_write, -1);

            double max_iter_value_read = get_max_iter_value(COLUMN_READ_ID);
            double max_iter_value_write = get_max_iter_value(COLUMN_WRITE_ID);
